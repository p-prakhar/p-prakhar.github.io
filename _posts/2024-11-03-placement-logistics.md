...
<style>
/* Initial font size for the content */
.zoom-content {
    font-size: 18px;
    line-height: 1.6;
}
...
<!-- Content Section with Zoom Capability -->
...
<script>
/* JavaScript functions to control zoom */
function zoomIn() {
...
function zoomOut() {
    let content = document.querySelector('.zoom-content');
    let currentSize = parseFloat(window.getComputedStyle(content).fontSize);
    if (currentSize > 12) { // Prevent font from getting too small
        content.style.fontSize = (currentSize - 2) + 'px';
    }
}
</script>
